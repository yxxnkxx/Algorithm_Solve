	...
	static void find() {
		...
		for (int i = 0; i < pick1.size(); i++) {
			if (i < 3) {
				...
				time[i] = Math.max(pick1.get(i).e1 + 1, time[i - 3]) + s1[2]; // 내가 거기까지 가는 거리 + i-3번째가 // 완료하는 시간
		...
		for (int i = 0; i < pick2.size(); i++) {
			...
			if (i < 3)
				...
				time[i + size1] = Math.max(pick2.get(i).e2 + 1, time[i + size1 - 3]) + s2[2]; // 내가 거기까지 가는 거리 + i-3번째가
			// 완료하는 시간
		}
