...
파이프 옮기기 1 과 조건은 똑같지만, 입력이 커져서 단순 dfs로는 해결하기 어려운 문제다.



처음에는 dp를 2차원 배열로 구현하여 파이프가 놓인 방향을 고려하지 않아 경우의 수가 잘못 나왔다.



그래서 가로, 세로, 대각선을 각각의 좌표마다 배열을 선언해서 3차원 배열로 dp를 진행하였다.



그리고 long으로 선언하지 않으면 입력값이 커질 때 올바른 답이 나오지 않는다.



해당 좌표의 dp값은 그 방향에서 왼쪽, 위, 대각선에서 가능한 경우의 수의 합이 된다.
...
map의 좌표가 1이 아닐 때에만 함수를 진행한다. 가로일 때는 이전 방향이 가로일 때와 대각선일 때, 세로일 때는 이전 방향이 세로, 대각선 그리고 대각선일 때는 모든 방향에서 올 수 있다.
