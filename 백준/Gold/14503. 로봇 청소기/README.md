...
처음에 테케가 이해가 안 돼서 어떻게 돌아가는 로직인지 헷갈렸다.

후진을 하는 방식을 잘못 이해해서 답이 다르게 나왔다.

이미 청소한 곳으로는 후진을 하지 않는다고 이해했는데, 그게 아니고 벽을 만날 때까지 후진해야 하는 문제였다.

벽과 이미 청소한 곳을 구분하기 위해 cnt를 2부터 시작하여 청소한 곳에 cnt를 저장하고, 마지막에 cnt-1을 값으로 출력하였다.

그리고 dfs로 한 번 방향을 찾으면 break를 하여 다른 방향으로 가지 못하도록 해야 한다.

그래야 후진을 통해서만 다음 청소할 곳을 찾을 수 있다.
