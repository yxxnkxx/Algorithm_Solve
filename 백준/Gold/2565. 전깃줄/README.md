...
위치의 번호는 500 이하의 자연수니까 크기가 501인 배열을 선언하였다.

A는 인덱스로, B는 그 인덱스의 값으로 배열을 채웠다.

또 dp\[A\]값을 1로 미리 정해놓고, LIS를 구할 때 1인 index에 대해서만 dp를 업데이트하였다.

dp값을 구하는 과정은 이전 문제들에서 사용했던 방식과 같다.
