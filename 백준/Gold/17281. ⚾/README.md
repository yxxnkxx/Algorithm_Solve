...
순열을 활용해서 타자들의 순서를 정하고,



그대로 게임을 한 후 최대값을 찾으면 된다.



시간 초과때문에 여러번 수정을 거쳤다.



처음에는 타자들의 순서를 Queue에 넣어 하나씩 빼고 다시 offer하는 식으로 하였는데, 그냥 배열의 index로 순환하는 것으로 수정하였다.



그리고 진루하는 과정에서 홈런을 친 경우에는 필드에 있는 모든 타자가 홈으로 돌아오는 것이니까 하나씩 더해주지 않고 field에 있는 타자의 수 만큼 점수에 더하였다. 이 과정에서 시간을 단축할 수 있었다.
