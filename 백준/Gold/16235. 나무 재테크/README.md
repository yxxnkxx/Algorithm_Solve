...
봄 여름 가을 겨울 순서대로 K번 반복한다.

나무를 2차원 배열의 index에 각각 리스트로 구현하고, 리스트에는 나이를 저장하였다.

봄에는 나이가 어린 나무부터 양분을 먹기 때문에 정렬을 하고 진행한다.

처음에 봄과 여름을 같이 처리하다가 list의 index문제와 양분이 즉시 더해지는 문제 때문에 테스트케이스에서 틀렸다.

그래서 tmp 배열에 여름에 추가할 양분을 저장하고, list에서 해당 값을 삭제한 후 for문의 변수인 t를 1 감소시키는 방법으로 봄과 여름을 처리하였다.

가을에는 나이가 5의 배수인 나무의 주변에 팔방탐색을 통해 새로운 나무를 추가하였다.

겨울에는 미리 저장한 A배열의 각 값을 더해준다.

마지막으로 각 Tree List의 크기를 cnt에 더하면서 tree의 수를 계산하였다.
