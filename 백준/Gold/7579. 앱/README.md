...
배낭 문제처럼 풀면 되지만, 최소값을 구해야하기 때문에 방법을 생각하기가 어려웠다.

배낭 문제는 최소의 무게로 최대의 가치를 만들어낼 수 있는 조합을 찾는 것이라면, 이 문제는 M바이트를 확보하기 위한 최소의 비용을 계산하는 것이기 때문이다.

그러나 표현 방식만 달랐을 뿐 최소의 비용으로 최대의 메모리를 확보하는 조합을 찾는 것으로, 배낭 문제와 유사한 방식으로 풀었다.

배열에 row는 i번째의 값, col은 cost, map[row][col]은 해당 비용으로 확보할 수 있는 메모리의 최대값을 저장하였다.

마지막에 배열을 돌며 map의 값이 M이상일 경우 해당 비용을 답으로 정하였다.
