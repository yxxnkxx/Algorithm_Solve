...
### 출력 
 ...
 ---
 bfs로 구현해야 한다는 건 바로 생각해냈다. 인접한 노드들을 방문할 때 day는 1씩 증가하기 때문이다.

그러나 기존의 bfs를 활용하면 하루에 2개 이상의 토마토가 익어 있을 경우 day를 세기가 어렵다.

그래서 인접한 토마토들을 익은 토마토의 값에 1씩 더해주며 bfs를 진행하였다. 이 과정을 반복하면 토마토의 값 중 가장 큰 값이 모든 토마토가 익기까지 걸린 시간이 될 것이다.

또 day의 초기값을 -1로 하고, bfs가 끝나면 0이 있는지를 체크하여 0이 없을 때에만 day를 cnt로 업데이트하였다.
