...
스도쿠를 채우는 문제인데, 1~9까지의 수를 찾은 후에 해당 답이 틀렸을 경우 백트래킹을 하는 과정을 짜는 게 어려웠다. 

백트래킹에 대해 다 이해하지 못해서인지 좀 더 공부가 필요할 것 같다

알맞은 수를 찾는 find함수를 정의하고 수를 찾지 못했을 경우 이전의 depth로 돌아가는 과정을 반복하도록 dfs를 짰다.
