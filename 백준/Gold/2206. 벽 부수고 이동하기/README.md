...
### 풀이
bfs를 활용하면 되는데, 벽을 부수고 이동할 때와 그렇지 않을 때의 방문 배열을 구분하여야 하기 때문에 visited 배열을 3차원으로 구성하였다.

만약 한 번 벽을 부수고 해당 위치를 방문할 경우, 그 벽을 부수지 않고 방문했을 때가 최단경로에 포함될 수 있기 때문에 두 경우의 수를 분리해야 한다.
